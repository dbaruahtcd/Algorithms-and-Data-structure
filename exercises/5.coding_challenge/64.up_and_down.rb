...
def arrange(string)
  ...
  # end
    if i % 2 == 0
      swap(arr, i, i+1) if arr[i].length > arr[i+1].length
    else
      swap(arr, i, i+1) if arr[i].length < arr[i+1].length
    end
    i += 1
...
str1 = 'who hit retaining The That a we taken' #who RETAINING hit THAT a THE we TAKEN
puts arrange(str1)

str2 = 'turn know great-aunts aunt look A to back' #turn GREAT-AUNTS know AUNT a LOOK to BACK
puts arrange(str2)
